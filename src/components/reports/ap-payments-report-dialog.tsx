...
export default function APPaymentsReportDialog() {
    const { openDialogs, closeDialog, openDialog, setDialogData } = useDialog();
    const [fromDate, setFromDate] = useState<Date | undefined>(new Date());
    const [toDate, setToDate] = useState<Date | undefined>(new Date());

    const handleRunReport = () => {
        setDialogData('ap-payments-report' as any, { fromDate, toDate });
        closeDialog('ap-payments-report-dialog' as any);
        openDialog('ap-payments-report' as any);
    };

    return (
        <Dialog open={openDialogs['ap-payments-report-dialog'] || false} onOpenChange={() => closeDialog('ap-payments-report-dialog' as any)}>
            <DialogContent className="max-w-md">
                <DialogHeader>
                    <DialogTitle>AP Payments Report</DialogTitle>
                </DialogHeader>
                <div className="p-6 space-y-4">
                    ...
                    <div className="pt-4 border-t flex justify-end gap-2">
                        <Button variant="outline" onClick={() => closeDialog('ap-payments-report-dialog')}
                        >
                            Cancel
                        </Button>
