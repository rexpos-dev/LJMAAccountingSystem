...
export default function ConsolidatedReportsDialog() {
    const { openDialogs, closeDialog, openDialog, setDialogData } = useDialog();
    const [fromDate, setFromDate] = useState<Date | undefined>(new Date());
    const [toDate, setToDate] = useState<Date | undefined>(new Date());

    const handleRunReport = () => {
        setDialogData('consolidated-report', { fromDate, toDate });
        closeDialog('consolidated-reports-dialog');
        openDialog('consolidated-report');
    };

    return (
        <Dialog open={openDialogs['consolidated-reports-dialog']} onOpenChange={() => closeDialog('consolidated-reports-dialog')}>
            <DialogContent className="max-w-md">
                <DialogHeader>
                    <DialogTitle>Consolidated Reports</DialogTitle>
                </DialogHeader>
                <div className="p-6 space-y-4">
                    ...
                    <div className="pt-4 border-t flex justify-end gap-2">
                        <Button variant="outline" onClick={() => closeDialog('consolidated-reports-dialog')}
                        >
                            Cancel
                        </Button>
