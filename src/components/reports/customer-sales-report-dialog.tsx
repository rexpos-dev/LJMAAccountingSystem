...
export default function CustomerSalesReportDialog() {
    const { openDialogs, closeDialog, openDialog, setDialogData } = useDialog();
    const [fromDate, setFromDate] = useState<Date | undefined>(new Date());
    const [toDate, setToDate] = useState<Date | undefined>(new Date());

    const handleRunReport = () => {
        setDialogData('customer-sales-report', { fromDate, toDate });
        closeDialog('customer-sales-report-dialog');
        openDialog('customer-sales-report');
    };

    return (
        <Dialog open={openDialogs['customer-sales-report-dialog']} onOpenChange={() => closeDialog('customer-sales-report-dialog')}>
            <DialogContent className="max-w-md">
                <DialogHeader>
                    <DialogTitle>Customer Sales Report</DialogTitle>
                </DialogHeader>
                <div className="p-6 space-y-4">
                    ...
                    <div className="pt-4 border-t flex justify-end gap-2">
                        <Button variant="outline" onClick={() => closeDialog('customer-sales-report-dialog')}
                        >
                            Cancel
                        </Button>
