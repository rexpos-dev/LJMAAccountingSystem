...
export default function OrdersReportDialog() {
    const { openDialogs, closeDialog, openDialog, setDialogData } = useDialog();
    const [fromDate, setFromDate] = useState<Date | undefined>(new Date());
    const [toDate, setToDate] = useState<Date | undefined>(new Date());

    const handleRunReport = () => {
        setDialogData('orders-report', { fromDate, toDate });
        closeDialog('orders-report-dialog');
        openDialog('orders-report');
    };

    return (
        <Dialog open={openDialogs['orders-report-dialog']} onOpenChange={() => closeDialog('orders-report-dialog')}>
            <DialogContent className="max-w-md">
                ...
                <div className="p-6 space-y-4">
                    ...
                    <div className="pt-4 border-t flex justify-end gap-2">
                        <Button variant="outline" onClick={() => closeDialog('orders-report-dialog')}
                        >
                            Cancel
                        </Button>
