...
export default function SalespersonReportDialog() {
    const { openDialogs, closeDialog, openDialog, setDialogData } = useDialog();
    const [fromDate, setFromDate] = useState<Date | undefined>(new Date());
    const [toDate, setToDate] = useState<Date | undefined>(new Date());

    const handleRunReport = () => {
        setDialogData('salesperson-report' as any, { fromDate, toDate });
        closeDialog('salesperson-report-dialog' as any);
        openDialog('salesperson-report' as any);
    };

    return (
        <Dialog open={openDialogs['salesperson-report-dialog'] || false} onOpenChange={() => closeDialog('salesperson-report-dialog' as any)}>
            <DialogContent className="max-w-md">
                <DialogHeader>
                    <DialogTitle>Salesperson Report</DialogTitle>
                </DialogHeader>
                <div className="p-6 space-y-4">
                    ...
                    <div className="pt-4 border-t flex justify-end gap-2">
                        <Button variant="outline" onClick={() => closeDialog('salesperson-report-dialog')}
                        >
                            Cancel
                        </Button>
