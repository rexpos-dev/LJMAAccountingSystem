...
export default function CashFlowStatementDialog() {
    const { openDialogs, closeDialog, openDialog, setDialogData } = useDialog();
    const [fromDate, setFromDate] = useState<Date | undefined>(new Date());
    const [toDate, setToDate] = useState<Date | undefined>(new Date());

    const handleRunReport = () => {
        setDialogData('cash-flow-report', { fromDate, toDate });
        closeDialog('cash-flow-statement-dialog');
        openDialog('cash-flow-report');
    };

    return (
        <Dialog open={openDialogs['cash-flow-statement-dialog']} onOpenChange={() => closeDialog('cash-flow-statement-dialog')}>
            <DialogContent className="max-w-md">
                ...
                <div className="p-6 space-y-4">
                    ...
                    <div className="pt-4 border-t flex justify-end gap-2">
                        <Button variant="outline" onClick={() => closeDialog('cash-flow-statement-dialog')}
                        >
                            Cancel
                        </Button>
