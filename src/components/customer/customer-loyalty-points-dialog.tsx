...
export default function CustomerLoyaltyPointsDialog() {
    ...
    return (
        <Dialog open={openDialogs['customer-loyalty-points']} onOpenChange={(open) => !open && closeDialog('customer-loyalty-points')}>
            <DialogContent className="max-w-[95vw] h-[90vh] flex flex-col">
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center gap-2 mb-3">
                    <div className="flex flex-wrap items-center gap-2">
                        <Button variant="outline" size="sm" onClick={() => toast({ title: 'Edit' })}>
                            <Pencil className="mr-2 h-4 w-4" /> Edit
                        </Button>
                        <Button variant="outline" size="sm" onClick={() => toast({ title: 'Remove' })}>
                            <Trash2 className="mr-2 h-4 w-4" /> Remove
                        </Button>
                        <Button variant="outline" size="sm" onClick={() => toast({ title: 'History' })}>
                            <History className="mr-2 h-4 w-4" /> History
                        </Button>
                        <Button variant="outline" size="sm" onClick={() => {
                            setDialogData('add-loyalty-card' as any, { selectedCustomerId: selectedRow?.customerId });
                            openDialog('add-loyalty-card' as any);
                        }}>
                            <User className="mr-2 h-4 w-4" /> Customer
                        </Button>
                        <Button size="sm" className="bg-primary hover:bg-primary/90" onClick={() => {
                            setDialogData('add-loyalty-points' as any, {
                                selectedCustomerId: selectedRow?.customerId,
                                loyaltyCardId: selectedRow?.loyaltyCardId,
                                pointSettingId: selectedRow?.pointSettingId,
                                mode: 'add'
                            });
                            openDialog('add-loyalty-points' as any);
                        }}>
                    ...
                    <div className="ml-0 md:ml-auto flex flex-wrap items-center gap-3 w-full md:w-auto mt-2 md:mt-0">
                        <Select>
                            ...
                            {loading ? (
                                ...
                                loyaltyPoints.map((point, index) => (
                                    ...
                                    >
                                        ...
                                        <TableCell>{point.customer?.customerName || 'N/A'}</TableCell>
                                        <TableCell>{balances[point.customerId] ?? point.totalPoints ?? 0}</TableCell>
                                        <TableCell>{point.totalPoints}</TableCell>
                                        <TableCell>{point.pointSetting?.description || 'N/A'}</TableCell>
                                        <TableCell>
