...
export function GenericRequestForm({ formName, initialData, mode = 'create', onSuccess, onCancel }: GenericRequestFormProps) {
    ...
    const form = useForm<FormValues>({
        ...
        defaultValues: {
            ...
            requestedBy: initialData?.requestedBy || (mode === 'create' ? currentUserName : ''),
            verifiedBy: initialData?.verifiedBy || '',
            approvedBy: initialData?.approvedBy || '',
            processedBy: initialData?.processedBy || '',
            releasedReceivedBy: initialData?.releasedReceivedBy || '',
        },
    });
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b pb-4 mb-4">
                        <div className="space-y-1">
                            ...
                            <p className="text-sm text-muted-foreground">Please fill out the details below to submit your request.</p>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Input {...field} placeholder="Enter name" disabled={isReadOnly} className="bg-muted/30 focus-visible:bg-transparent transition-colors" />
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            )}
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Input {...field} placeholder="Enter position" disabled={isReadOnly} className="bg-muted/30 focus-visible:bg-transparent transition-colors" />
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            )}
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel className="text-xs font-semibold uppercase text-muted-foreground">Business Unit / Dept</FormLabel>
                                    <FormControl>
                                        <Input {...field} placeholder="Enter unit" disabled={isReadOnly} className="bg-muted/30 focus-visible:bg-transparent transition-colors" />
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            )}
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem className="md:col-span-2">
                                    <FormLabel className="text-xs font-semibold uppercase text-muted-foreground">Purpose of Request</FormLabel>
                                    <FormControl>
                                        <Input {...field} placeholder="Reason for this request" disabled={isReadOnly} className="bg-muted/30 focus-visible:bg-transparent transition-colors" />
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            )}
                    ...
                    <div className="space-y-4 pt-4">
                        <div className="flex items-center justify-between">
                            <h3 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Itemized Details</h3>
                            {!isReadOnly && (
                        ...
                        <div className="border rounded-md overflow-hidden bg-muted/5">
                            <Table>
                                <TableHeader>
                                    <TableRow className="bg-muted/30 hover:bg-muted/30 font-medium"><TableHead className="w-[50px] text-center">#</TableHead><TableHead>Description</TableHead><TableHead className="w-[100px] text-center">Qty</TableHead><TableHead className="w-[140px] text-right">Unit Price</TableHead><TableHead className="w-[140px] text-right">Total</TableHead>{!isReadOnly && <TableHead className="w-[40px]"></TableHead>}</TableRow>
                                </TableHeader>
                                <TableBody>
                            ...
                            <div className="flex justify-between items-center px-4 py-3 bg-muted/20 border-t">
                                <span className="text-xs font-bold uppercase text-muted-foreground tracking-widest">Total Amount</span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 pt-6 border-t border-dashed">
                        {[
                            ...
                            { name: 'releasedReceivedBy', label: 'Released & Received:', options: staff },
                        ].map((sig) => (
                            <FormField
                                key={sig.name}
                                control={form.control}
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <p className="text-[9px] text-center text-muted-foreground mt-1 font-mono uppercase">Signature / Date</p>
                                    </FormItem>
                                )}
                            />
                        ))}
