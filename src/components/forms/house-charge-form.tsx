                        ...
                        <div className="border rounded-lg overflow-hidden shadow-sm">
                            <Table>
                                <TableHeader className="bg-muted/40">
                                    <TableRow className=""><TableHead className="w-[50px] text-[10px] font-bold uppercase text-center">No.</TableHead><TableHead className="text-[10px] font-bold uppercase">Description</TableHead><TableHead className="w-[80px] text-[10px] font-bold uppercase text-center">Qty</TableHead><TableHead className="w-[120px] text-[10px] font-bold uppercase text-right">Price</TableHead><TableHead className="w-[120px] text-[10px] font-bold uppercase text-right px-4">Total</TableHead>{!isReadOnly && <TableHead className="w-[40px]"></TableHead>}</TableRow>
                                </TableHeader>
                                <TableBody>
                                    {fields.map((field, index) => (
                                        <TableRow key={field.id} className="hover:bg-muted/5 group"><TableCell className="text-center font-mono text-[10px] text-muted-foreground">{index + 1}</TableCell><TableCell className=""><FormField control={form.control} name={`items.${index}.description`} render={({ field }) => (<FormControl><Input {...field} className="text-xs border-none bg-transparent focus-visible:ring-1 focus-visible:bg-background" placeholder="Item name..." disabled={isReadOnly} /></FormControl>)} /></TableCell><TableCell className=""><FormField control={form.control} name={`items.${index}.quantity`} render={({ field }) => (<FormControl><Input {...field} type="number" className="text-xs text-center border-none bg-transparent focus-visible:ring-1 focus-visible:bg-background" disabled={isReadOnly} /></FormControl>)} /></TableCell><TableCell className=""><FormField control={form.control} name={`items.${index}.unitPrice`} render={({ field }) => (<FormControl><Input {...field} type="number" step="0.01" className="text-xs text-right border-none bg-transparent focus-visible:ring-1 focus-visible:bg-background" disabled={isReadOnly} /></FormControl>)} /></TableCell><TableCell className="px-4 text-right text-xs font-bold">{(watchedItems[index]?.quantity * watchedItems[index]?.unitPrice || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</TableCell>{!isReadOnly && (<TableCell className="text-center"><Button type="button" variant="ghost" size="icon" className="w-6 text-muted-foreground hover:text-destructive" onClick={() => remove(index)} disabled={fields.length === 1}><Trash2 className="h-3.5 w-3.5" /></Button></TableCell>)}</TableRow>
                                    ))}
                                    <TableRow className="bg-muted/30 font-bold border-t-2"><TableCell colSpan={4} className="text-right text-[10px] uppercase tracking-wider">Grand Total</TableCell><TableCell className="text-right px-4 text-primary">Php {grandTotal.toLocaleString(undefined, { minimumFractionDigits: 2 })}</TableCell>{!isReadOnly && <TableCell></TableCell>}</TableRow>
                                </TableBody>
                            </Table>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-4 pt-4">
                        <FormField control={form.control} name="requestedBy" render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl><Input {...field} className="text-xs" disabled={isReadOnly} /></FormControl>
