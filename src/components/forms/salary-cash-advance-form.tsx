...
export function SalaryCashAdvanceForm({ initialData, mode = 'create', onSuccess, onCancel }: SalaryCashAdvanceFormProps) {
    ...
    const handlePrint = async () => {
        ...
        try {
            const canvas = await html2canvas(printRef.current, {
                ...
                onclone: (clonedDoc) => {
                    const clonedRoot = clonedDoc.getElementById('printable-form');
                    if (clonedRoot) {
                        ...
                        allElements.forEach(node => {
                            ...
                            if (el.style) {
                                // Force black text for everything that isn't supposed to be hidden
                                if (!el.classList.contains('no-print')) {
                                    el.style.setProperty('color', '#000000', 'important');
                                    // Handle nested backgrounds that might clash
                                    if (el.classList.contains('bg-white') || el.id === 'printable-form') {
                                        el.style.setProperty('background-color', '#ffffff', 'important');
                                    } else {
                                        el.style.setProperty('background-color', 'transparent', 'important');
                                    }

                                    // Ensure inputs and selects are also black
                    ...
                    // Explicitly hide elements with 'no-print' class in the PDF
            ...
            pdf.save(`Salary_Cash_Advance_${form.getValues('requestor') || 'Form'}.pdf`);
        } catch (error) {
    ...
    return (
        <div className="w-full max-w-4xl mx-auto bg-card rounded-lg shadow-sm print:shadow-none p-8 print:p-0">
            <div id="printable-form" ref={printRef} className="bg-white p-4 text-black">
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-10">
                                        ...
                                        render={({ field }) => (
                                            <Select value={field.value} onValueChange={(val) => {
                                                ...
                                                if (emp) {
                                                    ...
                                                    if (emp.designation) {
                                                        form.setValue('approverPosition', emp.designation, { shouldValidate: true });
                                                    }
                                                }
                                                // Synchronize with requestedBy signature name
                                                form.setValue('requestedBy', val, { shouldValidate: true });
                                ...
                                <div className="w-full space-y-3">
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem className="w-full text-center">
                                                <div className="w-full text-left self-start mb-2">
                                                    <p className="text-xs font-bold uppercase">PROCESSED BY:</p>
                                                </div>
                                                <Select value={field.value} onValueChange={field.onChange} disabled={isReadOnly}>
                                                    <FormControl>
                                                        <SelectTrigger className=" border-x-0 border-t-0 border-b-2 border-black rounded-none bg-transparent hover:bg-muted/10 transition-colors text-center font-bold text-lg px-0">
                        ...
                        <div className="flex justify-end gap-3 pt-8 border-t no-print">
                            ...
                            <Button type="button" variant="secondary" onClick={handlePrint} className="gap-2 shadow-sm" disabled={isSubmitting}>
